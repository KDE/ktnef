...
//-----------------------------------------------------------------------------
//@cond IGNORE
static QString stringProp(const KTNEFMessage *tnefMsg, quint32 key, const QString &fallback = QString())
{
    return tnefMsg->findProp(key < 0x10000 ? key & 0xFFFF : key >> 16, fallback);
}

static QString sNamedProp(const KTNEFMessage *tnefMsg, const QString &name, const QString &fallback = QString())
{
...
QString KTnef::messageToIcal(const KTNEFMessage *tnefMsg)
{
...
QByteArray KTnef::messageToVcard(const KTNEFMessage *tnefMsg)
{
    ...
    if (!msgClass.isEmpty()) {
        ...
        if (bCompatClassNote || "IPM.CONTACT"_L1 == msgClass) {
